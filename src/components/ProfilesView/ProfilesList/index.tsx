...
const columnHelper = createColumnHelper<IHasId<IProfile>>();

function getColumns(
  onClickEditOne: (_id: MongoId) => void,
  onClickDeleteOne: (_id: MongoId) => void,
  onClickDeleteSelected: (arg: RowModel<IHasId<IProfile>>) => void,
): ColumnDef<IHasId<IProfile>, any>[] {
  return [
    {
      id: 'select',
      header: ({ table }) => (
...
export default function ProfilesList() {
  ...
  function onEdit(item: IHasId<IProfile>) {
    router.push(`/dashboard/${teamId}/profiles/${item._id}`);
  }

  function onClickEditOne(_id: MongoId) {
    router.push(`/dashboard/${teamId}/profiles/${_id}`);
  }

  async function onClickDeleteOne(_id: MongoId) {
    if (confirm(CONFIRM_DELETE_ONE)) {
      await profiles.deleteItems([_id], teamId);
      setRowSelection({});
    }
  }

  async function onClickDeleteSelected(arg: RowModel<IHasId<IProfile>>) {
    const ids = arg.rows.map((v) => v.original._id);

    if (ids.length > 0 && confirm(CONFIRM_DELETE_SELECTED)) {
      await profiles.deleteItems(ids, teamId);
      setRowSelection({});
    }
  }

  function onClickNew() {
    router.push(`/dashboard/${teamId}/profiles/new`);
  }
  ...
  const hasItems = (profiles.data?.data || []).length > 0;
          ...
          <Card>
            <CardBody>
              <div className="flex flex-col space-y-3">
                ...
                <div>
                  <p className="text-center font-bold text-2xl">No Profiles</p>
                  <div className="flex justify-center mt-6">
                    <Button
                      ...
                      isInline
                    >
                      <PlusIcon className="me-1" /> New Profile
                    </Button>
                  </div>
                </div>
              </div>
